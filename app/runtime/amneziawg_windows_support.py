...
class AmneziaWGWindowsAssetLocator:
    def __init__(
        self,
        *,
        ...
        self._generated_dir = (
            Path(generated_dir) if generated_dir is not None else runtime_generated_dir() / "amneziawg" / "windows"
        )
        self._logs_dir = Path(logs_dir) if logs_dir is not None else runtime_logs_dir() / "amneziawg" / "windows"

    def locate(self) -> WireGuardRuntimeAssets:
        helper_path = self._helper_path or (
            self._engine_root_dir / "amneziawg" / "windows" / "proxyvault-amneziawg-windows.exe"
        )
        if not helper_path.exists():
            raise WireGuardAdapterError(
                WIREGUARD_FAILURE_HELPER_NOT_FOUND,
                last_error=f"AmneziaWG Windows helper was not found at {helper_path}.",
                log_excerpt=f"AmneziaWG Windows helper was not found at {helper_path}.",
            )
