...
class AmneziaWGMacOSAssetLocator:
    def __init__(
        self,
        *,
        ...
        self._generated_dir = (
            Path(generated_dir) if generated_dir is not None else runtime_generated_dir() / "amneziawg" / "macos"
        )
        self._logs_dir = Path(logs_dir) if logs_dir is not None else runtime_logs_dir() / "amneziawg" / "macos"

    def locate(self) -> WireGuardRuntimeAssets:
        helper_path = self._helper_path or (
            self._engine_root_dir / "amneziawg" / "macos" / "proxyvault-amneziawg-macos"
        )
        if not helper_path.exists():
            raise WireGuardAdapterError(
                WIREGUARD_FAILURE_HELPER_NOT_FOUND,
                last_error=f"AmneziaWG macOS helper was not found at {helper_path}.",
                log_excerpt=f"AmneziaWG macOS helper was not found at {helper_path}.",
            )
