...
class WireGuardWindowsAssetLocator:
    def __init__(
        self,
        *,
        ...
        self._generated_dir = (
            Path(generated_dir) if generated_dir is not None else runtime_generated_dir() / "wireguard" / "windows"
        )
        self._logs_dir = Path(logs_dir) if logs_dir is not None else runtime_logs_dir() / "wireguard" / "windows"

    def locate(self) -> WireGuardRuntimeAssets:
        helper_path = self._helper_path or (
            self._engine_root_dir / "wireguard" / "windows" / "proxyvault-wireguard-windows.exe"
        )
        if not helper_path.exists():
            raise WireGuardAdapterError(
                WIREGUARD_FAILURE_HELPER_NOT_FOUND,
                last_error=f"WireGuard Windows helper was not found at {helper_path}.",
                log_excerpt=f"WireGuard Windows helper was not found at {helper_path}.",
            )
